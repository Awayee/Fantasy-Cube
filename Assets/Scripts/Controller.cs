...
public class Controller : MonoBehaviour
{
    ...
    ///窗口管理
    ///<samary/>
    [SerializeField] private GameObject settingsPanel;//设置窗口
    [SerializeField] private GameObject sureExitPanel;//确认退出窗口
    [HideInInspector] public UnityEvent backButton;//返回键按钮事件
    ...
    void LateUpdate()
    {
        ...
        else if (Input.touchCount > 1)
        {
            ...
            if (Input.GetTouch(0).phase == TouchPhase.Ended) touchStart1 = Input.GetTouch(1).position;
            //if(Input.GetTouch(1).phase == TouchPhase.Ended)touchStart2 = Input.GetTouch(1).position;
